...
class SubtitleProcessor:
    ...
    async def process_subtitles(
        ...
        # Get or create video first and commit to get its ID
        video = await self._get_or_create_video(video_id, vtt_path, session)
        await session.flush()  # Make sure video is in DB and has an ID

        # Create user-video association
        assoc = UserVideoAssociation(
            user_id=user.id,
            video_id=video.id  # Now we have the video.id
        )
        session.add(assoc)
        await session.flush()

        # Extract and process bilingual lines
...
if __name__ == "__main__":
    ...
    async def test_processor():
        ...
        async with async_session() as session:
            ...
            await session.commit()  # Final commit for all changes
