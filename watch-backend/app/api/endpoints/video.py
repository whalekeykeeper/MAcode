...
from app.core.video_and_subtitles import get_bilingual_vtt, get_video_id
...
async def stream_video(
    video_id: str, session: AsyncSession = Depends(deps.get_session)
):
    """Streams the video with the specified video_id."""
    stmt = await session.execute(select(Video).where(Video.video_id == video_id))
